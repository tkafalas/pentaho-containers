...
public class DockerPentahoServerCli {
  ...
  public static void main( String[] args ) throws DockerPentahoException {
    ...
    try {
      new DockerPentahoServerService( false, combinedPentahoVersion, combinedPatchVersion, combinedPlugins,
        installPath, useExistingDownloads, noCache, port, database, karIds, productType, user, password, execute,
    ...
    Option optAdditionalPlugins = Option.builder( "A" )
      ...
      .desc(
        "Contains acronyms for the plugin products to be installed with the server. Known plugin types are \"std\", "
          + DockerPentahoUtil.arrayToUserFriendlyString( DockerPentahoServerService.KNOWN_PLUGINS )
